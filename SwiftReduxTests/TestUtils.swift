...
func ApplicationReducer(action: ActionType, state: BaseState) -> BaseState{
    ...
    return BaseState(
        counter: CounterReducer(action, state: state.counter),
        countries: CountryReducer(action, state: state.countries),
        textField: TextFieldReducer(action, state: state.textField)
    )
}
